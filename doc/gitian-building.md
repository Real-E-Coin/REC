...
*Setup instructions for a gitian build of Real_E_Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Real_E_Coin
...
to the Real_E_Coin GitHub release page.
...
- [Building Real_E_Coin](#building-real_e_coin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/real_e_coin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/real_e_coin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/real_e_coin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/real_e_coin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/real_e_coin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/real_e_coin-win-${VERSION%\.*}-build.assert
```
...
You can also mail the files to Fuzzbawls (fuzzbawls@real_e_coin.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/real_e_coin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/real_e_coin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/real_e_coin-osx-*-build.assert
```
