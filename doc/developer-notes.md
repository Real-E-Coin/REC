...
- [Development guidelines](#development-guidelines)
    - [General Real_E_Coin](#general-real_e_coin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Real_E_Coin uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play RECs (tREC)" on the test network, if you
...
Real_E_Coin is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/real_e_coin-project/real_e_coin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_real_e_coin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_real_e_coin --log_level=test_suite
$ valgrind -v --leak-check=full src/real_e_coind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_real_e_coin.coverage/index.html`.
```
...
- BitcoinMiner : Generates RECs (if wallet is enabled).
...
However, in open source software such as Real_E_Coin, where everyone uses
...
pay attention to for reviewers of Real_E_Coin code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Real_E_Coin uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Real_E_Coin so that they can be integrated
...
file descriptors it will cause problems with Real_E_Coin's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Real_E_Coin-Project/Real_E_Coin.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `real_e_coin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `real_e_coin-cli` and the GUI debug console use this table to determine how to
