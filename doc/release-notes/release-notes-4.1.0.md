...
Enabling major improvements over the keystore management, the Real_E_Coin wallet doesn't require regular backups as before, keys are following a deterministic creation path that can be verified at any time (before HD Wallet, every keypair was randomly created and added to the keypool, forcing the user to backup the wallet every certain amount of time or could end up loosing coins forever if the latest `wallet.dat` was not being used).
...
When the wallet contains no zREC, the zREC balance details will be hidden, reducing visual clutter.
...
```
"Confirmed","Date","Type","Label","Address","Amount (REC)","ID"
```
...
Automatic zREC backup has been disabled. Thus, the following configuration options have been removed  (either as entries in the real_e_coin.conf file or as startup flags):
- `autozrecbackup`
- `backupzrec`
- `zrecbackuppath`
...
The default value for the stake-split threshold has been lowered from 2000 REC, down  to 500 REC.
...
- "moneysupply" and "zrecSupply" attributes in `getblock` output are removed.
...
- The output of `getstakingstatus` was reworked. It now shows the following information:
  ```
  {
     ...
     "staking_enabled": true|false,      (boolean) whether staking is enabled/disabled in real_e_coin.conf
     "coldstaking_enabled": true|false,  (boolean) whether cold-staking is enabled/disabled in real_e_coin.conf
     "haveconnections": true|false,      (boolean) whether network connections are present
     "mnsync": true|false,               (boolean) whether masternode data is synced
     "walletunlocked": true|false,       (boolean) whether the wallet is unlocked
     "stakeablecoins": n,                (numeric) number of stakeable UTXOs
     "stakingbalance": d,                (numeric) REC value of the stakeable coins (minus reserve balance, if any)
...
### GUI
 ...
 - #1306 `65ba128634` [GUI] Hide privacy widget when the wallet has no zREC balance (furszy)
 - #1316 `bcb04a44a1` [Trivial] fix typo "recomended" in sendcustomfee dialog (random-zebra)
 - #1323 `e2c07184f9` [Qt] Hide zREC balances when they are zero (Fuzzbawls)
...
### Wallet
 ...
 - #1382 `df2db0d5c6` [Wallet] Don't initialize zrecwallet on first run (Fuzzbawls)
...
### Core Systems
 ...
 - #1308 `ca912fc823` [zREC] Public coin, a super for-each removed (furszy)
...
### Cleanup
 ...
 - #1290 `2ceeb2cca0` [zREC][Cleanup] Zerocoin Cleanup 1: remove Accumulators values (random-zebra)
 - #1291 `16d7dac5f7` [zREC][Cleanup] Zerocoin Cleanup 2: remove CZRecStake class (random-zebra)
 - #1293 `28e0048b3e` [zREC][Cleanup] Zerocoin Cleanup 3: remove old ZK proofs (random-zebra)
 - #1314 `185194bd7e` [zREC][Cleanup] Zerocoin Cleanup 4: further wallet cleaning (random-zebra)
 - #1322 `5bd387e18d` [zREC][Cleanup] Zerocoin Cleanup 5: further main.cpp cleaning (random-zebra)
 - #1326 `6bbd575860` [zREC][Cleanup] Zerocoin Cleanup 6: Remove zerocoin mint checks (random-zebra)
