...
Below are some notes on how to build Real_E_Coin for Windows.

The options known to work for building Real_E_Coin on Windows are:

* On Linux, using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Bionic 18.04 is required
and is the platform used to build the Real_E_Coin Windows release binaries.
...
Note that for WSL the Real_E_Coin source path MUST be somewhere in the default mount file system, for
example /usr/src/real_e_coin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\real_e_coin`, for example:

    make install DESTDIR=/mnt/c/workspace/real_e_coin
...
It's not possible to build the Real_E_Coin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Real_E_Coin source code).
