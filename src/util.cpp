...
#else
    const char* pszModule = "real_e_coin";
#endif
    if (pex)
        return strprintf(
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Real_E_Coin
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Real_E_Coin
// Mac: ~/Library/Application Support/real_e_coin
// Unix: ~/.real_e_coin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Real E Coin";
#else
    fs::path pathRet;
    ...
    return pathRet / "Real E Coin";
#else
    // Unix
    return pathRet / ".real_e_coin";
#endif
#endif
}
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Real_E_CoinParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Real_E_CoinParams
    // Mac: ~/Library/Application Support/Real_E_CoinParams
    // Unix: ~/.real_e_coin-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Real_E_CoinParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "Real_E_CoinParams";
#else
    // Unix
    return pathRet / ".real_e_coin-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty real_e_coin.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override real_e_coin.conf
