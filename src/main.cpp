...
#if defined(NDEBUG)
#error "Real_E_Coin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in urec) are considered zero fee (for relaying, mining and transaction creation)
...
{
	util::ThreadRename("real_e_coin-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> recInputs;

		for (const CTxIn& stakeIn : stakeTxIn.vin) {
			recInputs.push_back(stakeIn);
		}
		const bool hasRECInputs = !recInputs.empty();
		...
		for (const CTransaction& tx : block.vtx) {
			for (const CTxIn& in : tx.vin) {
				...
				if (hasRECInputs) {
					...
					for (const CTxIn& recIn : recInputs)
						if (recIn.prevout == in.prevout)
			...
			while (!chainActive.Contains(prev)) {
				...
				for (const CTransaction& t : bl.vtx) {
					...
					for (const CTxIn& in : t.vin) {
						...
						if (hasRECInputs) {
							for (const CTxIn& stakeIn : recInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman& connman, std::atomic<bool>& interruptMsgProc)
{
	...
	if (strCommand == NetMsgType::VERSION) {
		...
		// Real_E_Coin: We use certain sporks during IBD, so check to see if they are
