...
bool CRecStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CRecStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CRecStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CRecStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CRecStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CRecStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CRecStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CRecStake::GetUniqueness() const
{
    //The unique identifier for a REC stake is the outpoint
...
CBlockIndex* CRecStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CRecStake::ContextCheck(int nHeight, uint32_t nTime)
{
