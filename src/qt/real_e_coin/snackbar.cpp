...
#include "qt/real_e_coin/forms/ui_snackbar.h"
#include "qt/real_e_coin/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(Real_E_CoinGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &Real_E_CoinGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
