...
#include "qt/real_e_coin/dashboardwidget.h"
#include "qt/real_e_coin/forms/ui_dashboardwidget.h"
#include "qt/real_e_coin/sendconfirmdialog.h"
#include "qt/real_e_coin/txrow.h"
...
DashboardWidget::DashboardWidget(Real_E_CoinGUI* parent) :
	PWidget(parent),
	ui(new Ui::DashboardWidget)
{
	...
	setCssProperty(ui->labelSquareRec, "square-chart-rec");
	setCssProperty(ui->labelRec, "text-chart-rec");
	...
	setCssProperty(ui->labelAmountRec, "text-stake-rec-disable");
	...
	if (window)
		connect(window, &Real_E_CoinGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
	if (walletModel && walletModel->getOptionsModel()) {
		...
		connect(ui->pushImgEmpty, &QPushButton::clicked, window, &Real_E_CoinGUI::openFAQ);
		connect(ui->btnHowTo, &QPushButton::clicked, window, &Real_E_CoinGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
	...
	chartData->amountsByCache = getAmountBy(); // pair REC
	...
	for (int j = range.first; j < range.second; j++) {
		...
		if (chartData->amountsByCache.contains(num)) {
			...
			rec = (pair.first != 0) ? pair.first / 100000000 : 0;
			chartData->totalRec += pair.first;
		}

		chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

		chartData->valuesRec.append(rec);

		int max = std::max(rec, rec);
...
void DashboardWidget::onChartRefreshed()
{
	...
	set0->setColor(QColor(201, 161, 14));
	...
	set0->append(chartData->valuesRec);

	// Total
	nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
	if (chartData->totalRec > 0) {
		setCssProperty(ui->labelAmountRec, "text-stake-rec");
	}
	else {
		setCssProperty(ui->labelAmountRec, "text-stake-rec-disable");
	}
	forceUpdateStyle({ ui->labelAmountRec });
	ui->labelAmountRec->setText(GUIUtil::formatBalance(chartData->totalRec, nDisplayUnit));
