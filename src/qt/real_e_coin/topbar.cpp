...
#include "qt/real_e_coin/topbar.h"
#include "qt/real_e_coin/forms/ui_topbar.h"
#include "qt/real_e_coin/lockunlock.h"
#include "qt/real_e_coin/qtutils.h"
#include "qt/real_e_coin/receivedialog.h"
#include "qt/real_e_coin/loadingdialog.h"
...
TopBar::TopBar(Real_E_CoinGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopRec}, "amount-small-topbar");
    setCssProperty({ui->labelAmountRec}, "amount-topbar");
    setCssProperty({ui->labelPendingRec, ui->labelImmatureRec}, "amount-small-topbar");
...
void TopBar::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("Real E Coin")
    updateDisplayUnit();
...
void TopBar::updateBalances(const interfaces::WalletBalances& newBalance)
{
    ...
    // REC Total
    QString totalRec = GUIUtil::formatBalance(newBalance.balance, nDisplayUnit);

    // REC
    // Top
    ui->labelAmountTopRec->setText(totalRec);
    // Expanded
    ui->labelAmountRec->setText(totalRec);
    ui->labelPendingRec->setText(GUIUtil::formatBalance(newBalance.unconfirmed_balance, nDisplayUnit));
    ui->labelImmatureRec->setText(GUIUtil::formatBalance(newBalance.immature_balance, nDisplayUnit));
}
