...
class ReorgStakeTest(Real_E_CoinTestFramework):
    ...
    def check_money_supply(self, expected_bc, expected_zrec):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected REC and zREC supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_bc))
            for denom in node['zRECsupply']:
                assert_equal(node['zRECsupply'][denom], DecimalAmt(expected_zrec[denom]))
    ...
    def run_test(self):
        ...
        # Check REC and zREC supply at the beginning
        # ------------------------------------------
        # zREC supply: 2 coins for each denomination
        expected_zrec_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # REC supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zrec_supply)
        ...
        # Verify that REC and zREC supplies were properly updated after the spends and reorgs
        self.log.info("Check REC and zREC supply...")
        ...
        expected_zrec_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zrec_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zrec_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zrec_supply)
