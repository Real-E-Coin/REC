...
class ZerocoinSpendTest(Real_E_CoinTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zrec_bal, bc_bal):
            zrec_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zrec_bal)
            bc_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], bc_bal)
            return zrec_bal, bc_bal
        ...
        zrec_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zrec_balance, 6666)
        ...
        zrec_balance, balance = check_balances(denom_2, zrec_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zrec_balance, balance = check_balances(denom_3, zrec_balance, balance)
        ...
        _, _ = check_balances(denom_1, zrec_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
