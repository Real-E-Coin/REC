...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/Real_E_CoinParams"
    else
        PARAMS_DIR="$HOME/.real_e_coin-params"
    fi
fi
...
Failed to fetch the Real_E_Coin zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Real_E_Coin - fetch-params.sh

This script will fetch the Real_E_Coin zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Real_E_Coin zkSNARK parameters. Note that it is
